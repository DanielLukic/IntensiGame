...
public final class Real
    ...
    public void normalize()
        ...
        if ( ( this.exponent >= 0 ) )
            ...
            if ( mantissa > 0 )
                ...
                exponent++;
                if ( mantissa == 0 )
                    { // Ooops, it was 0xffffffffffffffffL
                    mantissa = 0x4000000000000000L;
                    exponent++;
                    }
                if ( exponent < 0 ) // Overflow
    ...
    private boolean testCarryWhenRounded( int base,
                                          int bitsPerDigit,
                                          int precision )
        {
        if ( mantissa >= 0 )
    ...
    private String ftoa( NumberFormat format )
        ...
        if ( ( this.exponent == 0 && this.mantissa == 0 ) )
            ...
            if ( format.base == 10 )
                ...
                int shift = bitsPerDigit - 1 -
                        floorMod( tmp4.exponent, bitsPerDigit );
                ...
                if ( shift == bitsPerDigit - 1 )
                    ...
                    if ( ( tmp4.sign != 0 ) )
                        ...
                        // (">>", not ">>>")
                        tmp4.mantissa |= 0x8000000000000000L >> ( shift - 1 );
                        }
                    }
                }
            }
        ...
        while ( precision > 0 )
            {
            ftoaBuf.append(
                    hexChar.charAt( (int) ( tmp4.mantissa >>> ( 64 - bitsPerDigit ) ) ) );
